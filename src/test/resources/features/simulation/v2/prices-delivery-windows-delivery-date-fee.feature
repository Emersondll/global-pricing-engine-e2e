@ok
Feature: I want to be able to validate types of prices with a delivery date fee instead charges from charges service applied in simulation/v2

  Scenario Outline:  Simulation V2 of prices with a delivery date fee instead charges from charges service
    ...
    * I have the requestTraceId header with value starting with a "e2e-put-price-v3"
    ...
    Given I have the payload at file "br-prices-v3-vendor1-type-priceList" at folder "price-service"
    * I have the "country" header with value "<country>"
    * I have the "timezone" header with value "<timezone>"
    * I have the x-timestamp header with current time
    * I have the requestTraceId header with value starting with a "e2e-put-price-v3"
    ...
    * I set the token to the vendorId "<vendorId>"
    ...
    Given I have the payload at file "br-prices-v3-vendor1-type-vendor" at folder "price-service"
    * I have the "country" header with value "<country>"
    * I have the "timezone" header with value "<timezone>"
    * I have the x-timestamp header with current time
    * I have the requestTraceId header with value starting with a "e2e-put-price-v3"
    ...
    * I set the token to the vendorId "<vendorId>"
    ...
    Scenarios:
      | country | timezone          | vendorId                             | vendorId_2                           | delivery-window-id-param                                                                                                    | account-file-name        | account2-file-name               | price-request-file-name          | price-file-name                          | delivery-windows-file        | simulation-file-name                                         |
      | BR      | America/Sao_Paulo | eafe490e-fddc-4b01-900a-d0dc45194ab6 | 42f582e0-eacb-49fe-95db-e35793615b2b | ## WILL BE REPLACED BY DELIVERY_WINDOW_ID WITH VENDOR_DELIVERY_WINDOW_ID 'DW-ID-FLEX-BEESPR-T4533' AND VENDOR KEY 'V-01' ## | br-account-simulation-v2 | br-account-simulation-v2-vendor2 | br-v3-post-request-with-one-item | br-prices-v3-simulation-v2-with-one-item | br-delivery-windows-flex-fee | br-simulation-v2-with-delivery-window-instead-charge-service |